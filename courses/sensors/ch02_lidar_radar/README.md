...
LiDAR fires light, radar fires radio waves. Both time the bounce, divide by twice the speed of the wave, and call the result distance. Light is small wavelength (≈905 nm typical for LiDAR), so you get sharp angular resolution but the beam is blocked by dust, rain, and fog. Radio waves are millimeter-to-centimeter wavelength (24–81 GHz for automotive radar), so they punch through weather but blur small objects together.
...
**Senses.** Time-of-flight of an infrared laser pulse (or, on cheaper units, the *angle* at which the reflection lands on a sensor — laser triangulation).

**Input.** 5 V power, no trigger needed (free-running).

**Output.** A 360° array of (angle, distance) pairs at 5–15 Hz.

**Integration.**
- **Physical interface:** USB serial (most hobby units) or UART + 5 V
...
- **Triangulation units (cheap A1/A2) lose accuracy past ~6 m**; ToF units (S2, C1) hold accuracy to 25–30 m but cost 3–4×
...
| [Hokuyo UST-10LX](https://www.hokuyo-usa.com/products/lidar-obstacle-detection/ust-10lx) | Industrial | ToF | 10 m / 20 m | ~$2,000 | AGV / AMR with safety requirements, 270° FoV, 0.25° resolution |
...
**Input.** 12–24 V power, gigabit Ethernet for the data firehose, GPS PPS pin if you need precise timestamps.

**Output.** A point cloud — millions of (x, y, z, intensity, ring, timestamp) tuples per second.
...
- **Non-ROS:** vendor SDKs (Ouster SDK in Python/C++, Velodyne PCAP tools), PCL, Open3D for processing
...
- **Vertical FoV is narrow** (15°–90° depending on model). Don't expect to see the sky.
- **Motion compensation** matters at speed — the robot moves during one full revolution, so a naive point cloud is *skewed*. Drivers handle this if you give them odom/IMU. If you don't, your SLAM map has a wobble.
...
- **Beam-steering tech varies wildly** — MEMS mirrors (Innoviz, Cepton), optical phased arrays (Quanergy), flash LiDAR (Cepton flash, Continental). Each has different range/resolution/cost trade-offs.
...
**What it does.** Instead of pulsing, transmits a continuous laser whose frequency ramps up and down. By measuring the frequency shift of the reflected light, the sensor recovers **distance and velocity simultaneously** (Doppler).
...
**Integration.**
- **Physical interface:** UART / USB on dev boards; CAN bus on automotive units
...
**Limitations to watch out for.**
- **Poor angular resolution.** A typical automotive mmWave radar tells you "a thing is at 50 m, moving 20 m/s, somewhere in this 5° cone." That's much fuzzier than LiDAR.
...
1. Project the LiDAR point cloud into the camera image plane using calibrated extrinsics
